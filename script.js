...
/* ====================================================================
   ...
   Data is stored in proper relational tables (not one JSON blob):
     - machines           one row per machine (master data)
     - breakdown_events   one row per failure/repair event
     - machine_documents  one row per uploaded document (manual, etc),
                           file itself lives in Supabase Storage
   ...
   ==================================================================== */
const SUPABASE_URL = 'https://oqjotrbrwjqwdevicmec.supabase.co';
const SUPABASE_KEY = 'sb_publishable_ZpVK3J95QH0RF9Ml4qnydA_I7BkUVm-';
...
/* ====================================================================
   ...
   Every signed-in user has identical full access — there are no
   separate roles/permission tiers. Add a login for someone in the
   Supabase Dashboard (Authentication -> Add user) and they can use
   the whole app immediately.
   ==================================================================== */
/* ---- Auth state (managed by auth.js lamp module) ---- */
let CURRENT_USER = null; // { id (uuid), email, displayName }

/* Called by auth.js after successful sign-in */
async function onLoginSuccess(user){
  const displayName = (user.email || '').split('@')[0] || 'User';
  CURRENT_USER = { id: user.id, email: user.email || '', displayName };
  authHideLoginScreen();
  await enterApp();
}
...
async function doLogout(){
  CURRENT_USER = null;
  STATE.machines = []; STATE.events = []; STATE.documents = []; STATE.masters = {areas:[],lines:[],customers:[]};
  sgsRealtimeChannels.forEach(ch => { try{ supabaseClient.removeChannel(ch); }catch(e){} });
  sgsRealtimeChannels = [];
  document.getElementById('appShell').style.display = 'none';
  await authSignOut(); // from auth.js — signs out + resets lamp UI
}

/* Reveal the app shell and bootstrap data + realtime. */
async function enterApp(){
  document.getElementById('appShell').style.display = 'flex';
  document.getElementById('sidebarUserName').textContent = CURRENT_USER ? CURRENT_USER.displayName : '—';
...
/* Check for a persisted session on page load (keeps user logged in after refresh). */
async function checkExistingSession(){
  if(!supabaseReady) return false;
  try{
    const user = await authCheckSession(); // from auth.js
...
/* React to background session changes (token refresh, revocation, expiry). */
function wireAuthStateListener(){
  if(!supabaseReady) return;
  authListenStateChanges((event, session) => {
...
function documentRowToApp(r){
  return { id: r.id, machineId: r.machine_id, fileName: r.file_name,
           storagePath: r.storage_path, fileSize: r.file_size,
           contentType: r.content_type, uploadedBy: r.uploaded_by,
           uploadedByName: r.uploaded_by_name, createdAt: r.created_at };
}

/* Build the Area/Line/Customer dropdown lists straight from whatever
   values are already in use on existing machines. There is no separate
   admin-managed master-data table anymore — the lists simply grow as
   new Area/Line/Customer values are typed in while adding/editing a
   machine. */
function rebuildMastersFromMachines(){
  const masters = {areas:[], lines:[], customers:[]};
  const seen = {areas:new Set(), lines:new Set(), customers:new Set()};
  STATE.machines.forEach(m=>{
    [['areas','area'],['lines','line'],['customers','customer']].forEach(([key,field])=>{
      const v = (m[field]||'').trim();
      if(v && !seen[key].has(v.toLowerCase())){
        seen[key].add(v.toLowerCase());
        masters[key].push(v);
      }
    });
  });
...
async function loadFromSupabase(){
  if(!supabaseReady) return false;
  try{
    const [machinesRes, eventsRes, documentsRes] = await Promise.all([
      supabaseClient.from('machines').select('*'),
      supabaseClient.from('breakdown_events').select('*'),
      supabaseClient.from('machine_documents').select('*')
    ]);
    ...
    if(documentsRes.error){ console.error('Supabase documents load error', documentsRes.error); return false; }

    isApplyingRemoteState = true;
    STATE.machines = (machinesRes.data || []).map(machineRowToApp);
    STATE.events = (eventsRes.data || []).map(eventRowToApp);
    STATE.documents = (documentsRes.data || []).map(documentRowToApp);
    rebuildMastersFromMachines();
...
/* Push current STATE up to Supabase (debounced so rapid local edits batch into one write).
   Uses upsert (insert-or-update) on primary keys for machines/events.
...
/* Explicit, targeted delete — called only from deleteMachine(), never
   inferred from "what's missing locally". */
async function deleteMachineRemote(machineId){
  if(!supabaseReady) return;
  try{
    // breakdown_events AND machine_documents both have ON DELETE CASCADE
    // on machine_id, so deleting the machine row also removes its events
    // and document records server-side; we still clear them from local
    // STATE/cache immediately for a snappy UI.
...
function subscribeSupabaseRealtime(){
  ...
  ['machines','breakdown_events','machine_documents'].forEach(table=>{
...
/* (Audit trail and password-change-request systems have been removed —
   this app no longer has an admin-only area, so there is nothing that
   needs a separate activity log or request queue.) */

/* ---------------- Global State ---------------- */
let STATE = {
  machines: [],   // {id, area, line, customer, machine, assetNo, status, qrPayload}
  events: [],     // {id, machineId, failTs, repairTs, status:'open'|'closed'}
  documents: [],  // {id, machineId, fileName, storagePath, fileSize, contentType, uploadedByName}
...
/* ====================================================================
   MACHINE DOCUMENTS (manuals, warranty cards, etc.)
   Files are stored in Supabase Storage (bucket: machine-documents);
   this just tracks metadata + uploads/downloads/deletes the file.
   ==================================================================== */
function getDocumentsForMachine(machineId){
  return STATE.documents.filter(d => d.machineId === machineId);
}

function getDocumentPublicUrl(storagePath){
  if(!supabaseReady) return '';
  const { data } = supabaseClient.storage.from('machine-documents').getPublicUrl(storagePath);
...
async function uploadMachineDocument(machineId, file){
  if(!supabaseReady){
  ...
  try{
    ...
    if(uploadError){
      ...
      showToast('error','Upload Failed', uploadError.message || 'Could not upload the file. Please try again.');
      return null;
    }
    ...
    const row = {
      ...
      uploaded_by_name: CURRENT_USER ? CURRENT_USER.displayName : null
    };
    const { data, error } = await supabaseClient.from('machine_documents').insert(row).select().single();
    if(error){
      console.error('Document metadata insert error', error);
      showToast('error','Upload Failed','File uploaded but could not be recorded. Please try again.');
      return null;
    }
    const doc = documentRowToApp(data);
    STATE.documents.push(doc);
    return doc;
  }catch(e){
    console.error('Document upload exception', e);
    showToast('error','Upload Failed','Something went wrong while uploading. Please try again.');
    return null;
  }
}

async function deleteMachineDocument(docId){
  const doc = STATE.documents.find(d => d.id === docId);
  if(!doc) return;
  try{
    await supabaseClient.storage.from('machine-documents').remove([doc.storagePath]);
    const { error } = await supabaseClient.from('machine_documents').delete().eq('id', docId);
    if(error){ console.error('Document delete error', error); showToast('error','Delete Failed','Could not delete the document.'); return; }
    STATE.documents = STATE.documents.filter(d => d.id !== docId);
    showToast('success','Document Deleted','The document was removed.');
  }catch(e){
    console.error('Document delete exception', e);
...
function renderMachinesMaster(){
  ...
  if(rows.length === 0){
    tbody.innerHTML = `<tr><td colspan="11" class="table-empty">No machines match your filters.</td></tr>`;
    return;
  }
  tbody.innerHTML = rows.map(m=>`
    <tr>
      ...
      <td>${getDocumentsForMachine(m.id).length > 0
        ? `<button class="btn btn-sm" onclick="openEditMachineModal('${m.id}')">${getDocumentsForMachine(m.id).length} file${getDocumentsForMachine(m.id).length>1?'s':''}</button>`
...
function deleteMachine(id){
  ...
  openConfirmModal('Delete Machine?', 'This will permanently remove the machine, all its breakdown history, and any uploaded documents. This cannot be undone.', async ()=>{
    STATE.machines = STATE.machines.filter(m=>m.id!==id);
    STATE.events = STATE.events.filter(ev=>ev.machineId!==id);
    STATE.documents = STATE.documents.filter(d=>d.machineId!==id);
    localStorage.setItem(LS_KEYS.machines, JSON.stringify(STATE.machines));
    localStorage.setItem(LS_KEYS.events, JSON.stringify(STATE.events));
...
function openEditMachineModal(id){
  ...
  openModal(`
    ...
    <div class="field-row" style="flex-direction:column;gap:14px;">
      ...
      <div class="field">
        <label>Documents (manual, warranty card, etc.)</label>
        <input class="field-input" id="editMachineDocFile" type="file">
        <button class="btn btn-sm mt8" id="editMachineDocUploadBtn" type="button">Upload Document</button>
        <div id="editMachineDocList" class="mt8"></div>
      </div>
    </div>
    <div style="display:flex;gap:10px;justify-content:flex-end;margin-top:20px;">
      <button class="btn" onclick="closeModal()">Cancel</button>
  ...
  document.getElementById('editMachineDocUploadBtn').onclick = ()=>handleDocUploadClick(id);
  renderMachineDocList(id);
}

function renderMachineDocList(machineId){
  const container = document.getElementById('editMachineDocList');
  if(!container) return;
  const docs = getDocumentsForMachine(machineId);
  if(docs.length === 0){
    container.innerHTML = '<span class="text-sm text-muted">No documents uploaded yet.</span>';
    return;
  }
  container.innerHTML = docs.map(d => `
    <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;padding:7px 0;border-bottom:1px solid rgba(255,255,255,0.06);">
      <a href="${getDocumentPublicUrl(d.storagePath)}" target="_blank" rel="noopener" style="color:var(--accent-blue);font-size:13px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:260px;">${escapeHtml(d.fileName)}</a>
      <span class="text-sm text-muted" style="font-size:11px;">${formatFileSize(d.fileSize)}</span>
      <button class="btn btn-sm btn-danger" onclick="handleDocDeleteClick('${d.id}','${machineId}')" type="button">Delete</button>
    </div>`).join('');
}

async function handleDocUploadClick(machineId){
  const input = document.getElementById('editMachineDocFile');
  const file = input.files && input.files[0];
  if(!file){ showToast('error','No File Selected','Please choose a file to upload.'); return; }
  const btn = document.getElementById('editMachineDocUploadBtn');
  btn.disabled = true; btn.textContent = 'Uploading…';
  const doc = await uploadMachineDocument(machineId, file);
  btn.disabled = false; btn.textContent = 'Upload Document';
  if(doc){
    input.value = '';
    renderMachineDocList(machineId);
    showToast('success','Document Uploaded', `${file.name} has been attached to this machine.`);
  }
}

function handleDocDeleteClick(docId, machineId){
  openConfirmModal('Delete Document?', 'This will permanently remove the document. This cannot be undone.', async ()=>{
...
/* (The Settings / Master Data admin page has been removed. Area, Line,
   and Customer values are now derived automatically from existing
   machines — see rebuildMastersFromMachines() — and grow naturally as
   new values are typed into the Add/Edit Machine forms. There is no
   separate management screen for them, and intentionally no "Load
   Sample Data" or "Reset All Data" controls anywhere in the app.) */
...
/* (The Password Change Request workflow and the Audit Log view have
   been removed along with the admin-only area of the app. There is no
   in-app self-service credential change — add/manage logins directly
   in the Supabase Dashboard under Authentication.) */

/* ====================================================================
   LOGIN / LOGOUT WIRING
   Login UI events are handled by auth.js (lamp interaction + form).
   Here we only wire the sidebar logout button.
   ==================================================================== */
...
async function init(){
  ...
  // Initialize the lamp login module (auth.js) — pass supabaseClient + success callback
  if(supabaseReady){
    authInit(supabaseClient, onLoginSuccess);
    wireAuthStateListener();
  } else {
    // Show lamp screen still, but login button will show an error
    authInit(null, onLoginSuccess);
  }

  // Check for existing session — skip lamp if already authenticated
  ...
  // Keep live view fresh if currently open (re-render every 30s)
  setInterval(()=>{
